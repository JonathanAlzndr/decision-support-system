...
export default function PilihKriteriaPage() {
	...
	useEffect(() => {
		if (kriteriaData && kriteriaData.status === "success") {
			setKriterias(kriteriaData.data);
			const initialBobot = {};
			kriteriaData.data.forEach((k) => {
				initialBobot[k.kode] = "";
			});
			setBobotData(initialBobot);
		}
	}, [kriteriaData]);
	...
	const handleSubmit = async (e) => {
		...
		const bobot_custom = Object.entries(bobotData)
			.filter(([_, nilai]) => nilai !== "")
			.map(([kode, nilai]) => ({
				kriteria_kode: String(kode),
				nilai: Number(nilai) / 5,
			}));

		if (bobot_custom.length === 0) {
			alert("Mohon isi setidaknya satu kriteria");
			return;
		}
